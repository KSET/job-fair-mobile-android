...
class EventViewModelMapper @Inject constructor(
    ...
    fun map(event: Event) = EventViewModel(
        ...
        startTimeFormatted = dateTimeFormatter.formatToTime(event.startTime),
        endTimeFormatted = dateTimeFormatter.formatToTime(event.endTime),
        ...
        type = when (event) {
            is Presentation -> EventType.PRESENTATION
